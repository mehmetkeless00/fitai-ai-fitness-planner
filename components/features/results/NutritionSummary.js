...
export default function NutritionSummary({ plan }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {plan.macros && (
        <Card className="mt-4">
          <p className="text-sm text-slate-400 mb-3">Macronutrient Breakdown</p>
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-white">Protein</span>
              <span className="text-red-400 font-semibold">{plan.macros.protein?.percentage || 28}%</span>
            </div>
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <span className="text-yellow-400 font-semibold">{plan.macros.carbs?.percentage || 48}%</span>
            </div>
            <div className="w-full bg-dark-bg rounded-full h-2 overflow-hidden">
              <div
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <span className="text-green-400 font-semibold">{plan.macros.fat?.percentage || 24}%</span>
            </div>
            <div className="w-full bg-dark-bg rounded-full h-2 overflow-hidden">
              <div
      ...
      {plan.hydration && (
        <Card className="mt-4">
          <div className="text-center">
            <p className="text-slate-400 text-sm mb-2">Daily Hydration Goal</p>
            <p className="text-xl font-semibold text-sky-400">{plan.hydration}</p>
          </div>
        </Card>
      )}
      ...
      {plan.dailyHabitTips && plan.dailyHabitTips.length > 0 && (
        <Card className="mt-4 bg-gradient-to-r from-purple-500/10 to-blue-500/10 border-purple-500/20">
          <h3 className="font-semibold text-purple-400 mb-3">💡 Daily Habit Tips</h3>
      ...
      {plan.groceryList && Object.keys(plan.groceryList).length > 0 && (
        <Card className="mt-4">
          <h3 className="font-semibold text-green-400 mb-3">🛒 Shopping List</h3>
          <div className="space-y-3">
            {Object.entries(plan.groceryList).map(([category, items]) => (
              <div key={category}>
                <p className="text-sm font-semibold text-green-400 mb-1">{category}</p>
                <div className="flex flex-wrap gap-2">
                  {items.slice(0, 5).map((item, idx) => (
                    <span key={idx} className="text-xs bg-green-500/10 text-green-300 px-2 py-1 rounded">
                      {item}
                    </span>
                  ))}
                  {items.length > 5 && (
                    <span className="text-xs text-slate-400 px-2 py-1">+{items.length - 5} more</span>
                  )}
                </div>
              </div>
            ))}
