...
export function generatePlanPDF(planData) {
  ...
  doc.setFont(undefined, 'bold');
  doc.text('FitAI - Your Personal Fitness Plan', 20, 18);
  yPosition = 40;
  ...
  doc.setFontSize(9);
  doc.setTextColor(...colors.lightText);
  doc.text('Generated by FitAI - Your Personal Fitness Coach', 20, yPosition);
  doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 20, yPosition + 5);

  const filename = `FitAI-Plan-${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
}
