...
export default function WorkoutDay({ day, t, onExercisePress }) {
  ...
  return (
    <Card className="mb-3">
      ...
      >
        <View>
          <Text className="font-semibold text-slate-900 dark:text-white">{day.day}</Text>
          {day.focus && (
            <Text className="text-xs text-slate-400 mt-0.5">{day.focus}</Text>
          )}
        </View>
        <View className="flex-row items-center gap-2">
          {isRest ? (
            ...
            <>
              <Text className="text-xs text-slate-400">{day.exercises.length} exercises</Text>
